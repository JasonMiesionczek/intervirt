...
class Class:
    def __init__(self, name, properties, ns):
        self.name = name
...
def parse_class(block):
    ...
    return Class(name=name, properties=properties, ns=ns)
...
def main():
    if "srcdir" in os.environ:
        ...
        input_filename = os.path.join(os.getcwd(), "classes.input")
    ...
    uri_prefix = {
        'common': 'http://schemas.microsoft.com/wbem/wsman/1/wmi/root/cimv2/',
        'v1': 'http://schemas.microsoft.com/wbem/wsman/1/wmi/root/virtualization/',
        'v2': 'http://schemas.microsoft.com/wbem/wsman/1/wmi/root/virtualization/v2/'
    }
            ...
            for cls_name in names:
                ...
                if not found:
                    ...
                    for p in classes_by_name[cls_name].properties:
                        ...
                        if p.is_array:
                            fieldtype = p.type + '[]'
                        field = {'name': p.name, 'type': fieldtype}
                        newcls['fields'].append(field)
                    classes_to_add.append(newcls)

            print classes_to_add
            for class_to_add in classes_to_add:
                ns['classes'].append(class_to_add)
